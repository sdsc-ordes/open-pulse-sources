"""Adapter wrapping `open_pulse_sources.index.gitlab_datascience_groups` for federated search/lookup."""
...
class GitLabDatascienceGroupsAdapter:
    name = "gitlab_datascience_groups"
    entity_types: list[str] = ["group"]  # noqa: RUF012

    # Manifest hints (see IndexAdapter docstring).
    backend = "vector"
    surface_as_source = True
    id_shape = "url"  # group_id is already the canonical https://gitlab.datascience.ch/groups/... URL
    ...
    def search(
        ...
        *,
        ...
        try:
            from open_pulse_sources.index.gitlab_datascience_groups.retrieval import search  # noqa: PLC0415, I001
    ...
    def lookup(self, identifier: str) -> list[EntityRecord]:  # noqa: PLR0911
        ...
        if host != "gitlab.datascience.ch":
            return []
        try:
            from open_pulse_sources.index.gitlab_datascience_groups.store import open_store  # noqa: PLC0415, I001
        except Exception:  # noqa: BLE001
            return []
        store = None
        try:
...
register(GitLabDatascienceGroupsAdapter())
