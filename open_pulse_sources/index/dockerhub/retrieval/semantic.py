"""Semantic retrieval over the Docker Hub index.

Embed → Qdrant search → RCP rerank → DuckDB hydrate. Reuses the shared
...
from open_pulse_sources.index.dockerhub.embed.pipeline import DOCKERHUB_COLLECTION
        ...
        hydrated.append(
            {
                ...
                "payload": payload,
                "entity": store.fetch_image(str(repo_id)),
            },
        )
    return hydrated
...
def semantic_search(
    *,
    ...
    store: DockerhubStore | None = None,
) -> list[dict[str, Any]]:
    if store is None:
