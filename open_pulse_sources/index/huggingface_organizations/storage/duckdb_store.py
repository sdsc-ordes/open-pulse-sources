"""DuckDB store for the huggingface_organizations index."""
...
class HuggingFaceOrganizationsStore:
    """Thin DuckDB wrapper for the HF organizations schema."""
    ...
    @classmethod
    def open(cls, db_path: Path | None = None) -> HuggingFaceOrganizationsStore:
        if db_path is None:
            db_path = get_huggingface_organizations_paths().duckdb_path
        store = cls(db_path)
        store.bootstrap()
        return store
    ...
    def upsert_organization(self, record: HFOrgRecord) -> None:
        sql = (
    ...
    def fetch_organization(self, slug: str) -> dict[str, Any] | None:
        return fetch_one(
            self.connect(),
            table="organizations",
            id_column=ID_COLUMN,
