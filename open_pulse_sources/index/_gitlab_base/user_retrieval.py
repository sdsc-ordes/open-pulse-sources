"""Semantic retrieval over the GitLab users index.
...
datascience) can pass its own Qdrant collection and DuckDB store.
"""
        ...
        hydrated.append(
            {
                ...
                "payload": payload,
                "entity": store.fetch_user(str(user_id)),
            },
        )
    return hydrated


def user_semantic_search(
    *,
    config: Any,
    collection: str,
    store: GitLabUserStore,
    ...
    """Search GitLab users semantically in the given Qdrant collection."""
    return asyncio.run(
        _async_search(
            config=config,
