"""Qdrant client wrapper for the Infoscience index.

Per-entity collections following the canonical pattern in this repo:

    * ``infoscience_chunks``        — publication body fragments
    * ``infoscience_articles``      — one row per matched publication
    * ``infoscience_persons``       — Person entities resolved from author
                                       authority IDs
    * ``infoscience_organizations`` — OrgUnit entities
...
from .config import InfoscienceIndexConfig, QdrantConfig
from .models import (
    ArticleRecord,
    ChunkRecord,
    OrganizationRecord,
...
CHUNKS_COLLECTION = "infoscience_chunks"
ARTICLES_COLLECTION = "infoscience_articles"
PERSONS_COLLECTION = "infoscience_persons"
ORGANIZATIONS_COLLECTION = "infoscience_organizations"
...
# Stable namespace for UUIDv5 point IDs scoped to this index.
_INDEX_NAMESPACE = uuid.UUID("8a4f0e6b-3c11-4f25-9a1f-3a0e2c5b7d11")
...
def chunk_payload(rec: ChunkRecord) -> dict[str, Any]:
    return _drop_none({
        ...
        "infoscience_url": rec.infoscience_url,
...
def organization_payload(rec: OrganizationRecord) -> dict[str, Any]:
    return _drop_none({
        ...
        "name": rec.name,
        "acronym": rec.acronym,
        "infoscience_code": rec.infoscience_code,
        ...
        "parent_acronym": rec.parent_acronym,
        "director_name": rec.director_name,
        "org_type_dspace": rec.org_type_dspace,
...
class QdrantStore:
    ...
    @classmethod
    def from_config(cls, cfg: InfoscienceIndexConfig) -> "QdrantStore":
