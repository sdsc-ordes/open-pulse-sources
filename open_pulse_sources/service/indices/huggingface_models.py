"""Async ingest helper for the huggingface_models index."""
...
def get_or_create_huggingface_models_resources(app_state: Any) -> Any | None:
    cached = getattr(app_state, "v2_huggingface_models_resources", None)
    ...
    return app_state.v2_huggingface_models_resources
...
async def run_huggingface_models_ingest_job(
    *,
    ...
    try:
        ...
        resources = get_or_create_huggingface_models_resources(app_state)
        if resources is None:
            existing.status = IndexIngestJobStatus.FAILED
            existing.completed_at = datetime.now(timezone.utc)
            existing.error = "huggingface_models index module unavailable on this deployment"
            job_store.set(existing)
            return
        ...
        embed_summary = await run_embed_step(
            provider=INDEX_NAME,
            job_id=job_id,
            embed_call=lambda: embed_models(config=config, store=store),
        ...
        logger.exception("huggingface_models ingest job %s failed", job_id)
...
async def run_huggingface_models_search(
